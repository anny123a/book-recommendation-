...
def generate_purchase_decision(
    ...
    # Handle empty pastsales DataFrame
    if pastsales.empty:
        sales_summary = "No historical sales data available (first-time purchase)"
        avg_monthly_sales = 0
        avg_daily_sales = 0
        peak_avg_daily = 0
        non_peak_avg_daily = 0
    else:
        # Convert pastsales DataFrame to a readable string format
        sales_summary = pastsales.to_string(index=True)
...
#     isbn = '9788861827288' # user input  
#     breakeven = 24.01  # user input 
#     df = "Sales (1).csv" # user input 
#     print(isbn)

#     # Today's date and order date range
#     today = datetime.now()
#     start_date = "2026-01-10" # user input
#     end_date = "2026-01-20"   # user input
...
#     # Past sales data
#     pastsales =  sales_summary #if sales_summary.empty else sales_summary
